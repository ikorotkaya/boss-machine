...
describe('BONUS: /api/minions/:minionId/work routes', function () {
  ...
  describe('PUT /api/minions/:minionId/work/:workId', function () {
    ...
    it('updates the correct work item and persists to the database', function () {
      ...
      let persistedWork;
      let updatedWorkParams;
      return request(app)
        .post('/api/minions/2/work')
        .send(newWorkParams)
        .then((response) => persistedWork = response.body)
        .then(() => {
          updatedWorkParams = Object.assign({}, persistedWork, { title: 'Persistence Test' });
          return request(app)
            .put(`/api/minions/2/work/${persistedWork.id}`)
            .send(updatedWorkParams);
        })
        .then(() => {
          return request(app)
            .get(`/api/minions/2/work`);
        ...
        .then((response) => response.body)
        .then(allMinionWorks => {
          let updatedWork = allMinionWorks.find(work => work.id === persistedWork.id);
          expect(updatedWork).to.have.property('title', 'Persistence Test');
        })
    });
